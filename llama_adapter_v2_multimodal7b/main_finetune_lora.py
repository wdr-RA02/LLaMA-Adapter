...
def get_args_parser():
    ...
    parser.add_argument('--lora_rank', default=16, type=int,
                        help="rank of lora layer")
    parser.add_argument('--quant', choices=['8bit', '4bit', 'no'], default='no',
                        type=str, help="bits of quantization")

    return parser


def trainable_params(model, trainables: List[str]):
    print("Trainable modules: {}".format(trainables))
    for name, para in model.named_parameters():
        para.requires_grad = False

    for name, para in model.named_parameters():
        if name.startswith("llama."):
            if any(n in name for n in trainables):
